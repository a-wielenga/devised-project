...
root.state("zoomed")
root.minsize(800, 600)   # Prevent shrinking too small

root.grid_columnconfigure(0, weight=1)   # Sidebar
root.grid_columnconfigure(1, weight=6)   # Main content
...
dates = [f"Day {i+1}" for i in range(14)]  # 2 weeks
times = [f"{h}:00" for h in range(0, 24)]  # All hours of the day

for col, d in enumerate(dates):
    tk.Label(calendar_frame, text=d, borderwidth=1, relief="flat", height=2, width=12).grid(row=0, column=col+1)

for row, t in enumerate(times):
    tk.Label(calendar_frame, text=t, borderwidth=1, relief="flat", width=10).grid(row=row+1, column=0)
    
    for col in range(len(dates)):
        tk.Label(calendar_frame, text="", borderwidth=1, relief="solid", width=12, height=2).grid(row=row+1, column=col+1)
